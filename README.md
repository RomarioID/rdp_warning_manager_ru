...
Скрипт не содержит адресов серверов, логинов, паролей и команд подключения.

Он не запускает `mstsc`, не сохраняет учетные данные, не включает RDP-сервер на компьютере, не открывает порт `3389`, не меняет правила брандмауэра и не создает пользователей.

Единственное действие скрипта - управление локальным параметром клиента RDP `RedirectionWarningDialogVersion`. Этот параметр влияет только на показ предупреждения перед подключением с текущего компьютера. Он не дает другим людям доступ к вашему серверу и не публикует никакие данные о сервере.
